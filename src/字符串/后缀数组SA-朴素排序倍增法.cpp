...
/*
    ...
    3.求字符串的本质不同子串个数: SPOJ Link: https://vjudge.net/problem/SPOJ-SUBST1
*/
struct SuffixArray{//后缀数组
    int n;//字符串长度
    string S;//构建后缀数组的对象字符串S,1-based
    ...
    const char apart='#';//不计入后缀数组和后缀排序的分隔符(sa[i]=rank[i]=0不影响字典序和排序)

    SuffixArray(){}

    SuffixArray(const string& s){//构建字符串s的后缀数组
        S=apart+s;
        n=s.size();
        sa.assign(n+1,0);
        height.assign(n+1,0);
